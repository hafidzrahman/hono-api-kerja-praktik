...
export default class JadwalService {
  ...
  async createJadwal(data: CreateJadwalDto): Promise<jadwal> {
    const conflicts = await this.checkScheduleConflicts(data.tanggal, data.waktu_mulai, data.waktu_selesai, data.nama_ruangan);
  ...
  async checkScheduleConflicts(tanggal: Date, waktu_mulai: Date, waktu_selesai: Date, ruangan: string, excludeJadwalId?: string): Promise<ScheduleConflict> {
    const ruanganConflicts = await this.jadwalRepository.findConflictingSchedules(tanggal, waktu_mulai, waktu_selesai, ruangan, excludeJadwalId);

    const dosenConflicts = await this.jadwalRepository.findDosenConflicts(tanggal, waktu_mulai, waktu_selesai, excludeJadwalId);
