...
export default class CryptoHelper {
  ...
  private static crypto = new SimpleCrypto(
    process.env.SECRET_KEY_ID_PEMBIMBING_INSTANSI_VALIDATOR
  );
...
const ch = CryptoHelper.generateEncryptedIDByPayload(
  JSON.stringify({ email_pembimbing_instansi: "iwan.iskandar@uin-suska.ac.id" })
);
console.log(ch);
